(function(){
  ...
  ready(function(){
    let tries=0;
    const wait=setInterval(()=>{ if(window.Chart&&window.jspdf){clearInterval(wait);init();} else if(++tries>100){clearInterval(wait);init();}},40);
    ...
    function init(){
      ...
      async function exportPdf(){
        try{
          ...
          // Bottom card: bar chart (offscreen)
          const bc=document.createElement('canvas'); bc.width=900; bc.height=300; const bctx=bc.getContext('2d');
          new Chart(bctx,{type:'bar', data:{labels:UNITS, datasets:[{data:UNITS.map(u=>unitAvg(L,u)), backgroundColor:greenGrad(bctx), borderRadius:10}]},
