...
(function () {
  ...
  ready(function () {
    ...
    const wait = setInterval(() => {
      if (window.Chart && window.jspdf) { clearInterval(wait); init(); }
      else if (++tries > 100) { clearInterval(wait); init(); }
    }, 40);
    ...
    function init() {
      ...
      function unitAvg(l,u){ const m = unitMap(l,u); const vals = TRADES.map(t => parseInt((m[t]||'0').replace('%',''))||0); const s = Math.round(vals.reduce((a,b)=>a+b,0) / TRADES.length); return Math.max(0, Math.min(100, s)); }
      function levelAverages(l){ return UNITS.map(u => unitAvg(l,u)); }

      function grad(ctx, c1, c2){ const g = ctx.createLinearGradient(0, ctx.canvas.height, ctx.canvas.width, 0); g.addColorStop(0, c1); g.addColorStop(1, c2); return g; }
      function greenGrad(ctx){ return grad(ctx, "#86efac", "#16a34a"); }
      function redGrad(ctx){ return grad(ctx, "#fecaca", "#7f1d1d"); }

      let donut, levelBar; Chart.defaults.animation = { duration: 400 };
        ...
        donut = new Chart(ctx, {
          ...
          plugins: [{
            ...
            afterDatasetsDraw(chart){
              const d = chart.getDatasetMeta(0).data[0]; if(!d) return;
              const cx = chart.ctx; cx.save(); cx.textAlign="center"; cx.fillStyle="#e5e7eb"; cx.font="700 30px Inter, system-ui"; cx.fillText(`${avg}%`, d.x, d.y-6); cx.font="600 12px Inter, system-ui"; cx.fillText("COMPLETE", d.x, d.y+16); cx.restore();
            }
          }]
        });
      }
      ...
      function refresh(){ const L = levelSelect.value || LEVELS[0]; const U = unitSelect.value || UNITS[0]; renderDonut(L,U); renderTable(L,U); renderLevelBar(L); }
      ...
      exportBtn.addEventListener("click", async () => {
        try {
          ...
          const w = pdf.internal.pageSize.getWidth(); const M = 24;
          ...
          if (logo && (logo.complete || logo.naturalWidth)) { pdf.addImage(logo, "PNG", M+12, M+10, 40, 40); }
