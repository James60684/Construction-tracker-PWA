    ...
    function init(){
      ...
      async function exportPdf(){
        try{
          ...
          function unitAvg(L,u){const vals=TRADES.map(t=>parseInt((localStorage.getItem(key(L,u,t))||'0%'))||0); return Math.round(vals.reduce((a,b)=>a+b,0)/TRADES.length);}
          UNITS.forEach((u,i)=>{
            ...
            pdf.setTextColor(200); pdf.text(f"{unitAvg(L,u)}%", c2, y);
            pdf.text(maxTs or '-', c3, y);
            y+=rowH;
          });
          ...
          new Chart(bctx,{type:'bar', data:{labels:UNITS, datasets:[{data:(()=>{return UNITS.map(u=>{return (function(){const vals=TRADES.map(t=>parseInt((localStorage.getItem(key(L,u,t))||'0%'))||0); return Math.round(vals.reduce((a,b)=>a+b,0)/TRADES.length);})()})})(), backgroundColor:greenGrad(bctx), borderRadius:10}]},
